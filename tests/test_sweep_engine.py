...
class SweepEngineTests(unittest.TestCase):
    ...
    def test_both_sides_and_inside_is_neutral(self):
        df = self._fx([100, 101, 102, 103, 104, 105, 110, 90, 100, 100],
                      [95, 96, 97, 96, 95, 94, 90, 90, 96, 100],
                      [98, 99, 100, 99, 98, 97, 100, 100, 100, 100])
        r = detect_sweep(df, "EURUSD=X", datetime(2026, 8, 25, 11, 0, tzinfo=IST))
        self.assertIsNotNone(r)
        self.assertEqual(r.direction, "NEUTRAL")

    def test_touch_does_not_sweep(self):
        df = self._fx([100, 101, 102, 103, 104, 105, 100],
                      [95, 96, 97, 96, 95, 94, 90],
                      [98, 99, 100, 99, 98, 97, 98])
        r = detect_sweep(df, "EURUSD=X", datetime(2026, 8, 25, 11, 0, tzinfo=IST))
        self.assertIsNone(r)

    def test_close_above_is_buy(self):
        df = self._fx([100,101,102,103,104,105,110,111,112,113],
                      [95,96,97,96,95,94,90,91,92,93],
                      [98,99,100,99,98,97,107,107,107,107])
        r = detect_sweep(df, "EURUSD=X", datetime(2026, 8, 25, 11, 0, tzinfo=IST))
    ...
    def test_close_below_is_sell(self):
        df = self._fx([100,101,102,103,104,105,110,111,112,113],
                      [95,96,97,96,95,94,90,91,92,93],
                      [98,99,100,99,98,97,93,93,93,93])
        r = detect_sweep(df, "EURUSD=X", datetime(2026, 8, 25, 11, 0, tzinfo=IST))
    ...
    def test_incomplete_candle_is_not_used(self):
        ...
        r = detect_sweep(df, "GC=F", datetime(2026, 8, 25, 5, 30, tzinfo=IST))
        self.assertIsNone(r)

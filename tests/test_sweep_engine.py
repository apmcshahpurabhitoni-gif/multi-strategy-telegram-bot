...
class SweepEngineTests(unittest.TestCase):
    ...
    def test_correct_source_close_has_no_schedule_warning(self):
        df = self._fx([100,101,102,103,104,105,110,111,112,113], [95,96,97,96,95,94,90,91,92,93], [98,99,100,99,98,97,107,107,107,107])
        r = detect_sweep(df, "EURUSD=X", self._now(11, 0))
        self.assertIsNotNone(r)
        self.assertEqual(r.candle_end.strftime("%H:%M"), "10:30")
        self.assertIsNone(r.schedule_warning)

    def test_shifted_source_close_gets_prominent_schedule_warning(self):
        df = self._fx([100,101,102,103,104,105,110,111,112,113], [95,96,97,96,95,94,90,91,92,93], [98,99,100,99,98,97,107,107,107,107])
        shifted = list(df.index)
        shifted[-1] = IST.localize(datetime(2026, 8, 25, 10, 0))
        df.index = shifted
        r = detect_sweep(df, "EURUSD=X", self._now(11, 30))
        self.assertIsNotNone(r)
        self.assertEqual(r.candle_end.strftime("%H:%M"), "11:00")
        self.assertIn("CANDLE TIME WARNING", r.schedule_warning)
        self.assertIn("expected close 10:30 IST", r.schedule_warning)
        self.assertIn("received 11:00 IST", r.schedule_warning)

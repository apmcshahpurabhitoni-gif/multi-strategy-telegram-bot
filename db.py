...
SQLITE_DB_PATH = "/tmp/workspace/state.db"
...
class DatabaseManager:
        ...
        with sqlite3.connect(SQLITE_DB_PATH) as conn:
            ...
            cur.execute("""
                CREATE TABLE IF NOT EXISTS pending_sweeps (
                    ...
                    sweep_high REAL,
                    sweep_low REAL,
                    sweep_open_ts INTEGER,
                    sweep_close_ts INTEGER,
                    created_at TEXT,
                    fvg_zone_low REAL,
                    fvg_zone_high REAL,
                    fvg_tf TEXT,
    ...
    def update_account_balance(self, acc_name: str, pnl: float):
        with sqlite3.connect(SQLITE_DB_PATH) as conn:
            cur = conn.cursor()
            cur.execute("UPDATE accounts SET balance = balance + ? WHERE name = ?", (pnl, acc_name))
            conn.commit()

    def increment_daily_trades(self, acc_name: str):
        with sqlite3.connect(SQLITE_DB_PATH) as conn:
            cur = conn.cursor()
            cur.execute("UPDATE accounts SET daily_trades = daily_trades + 1 WHERE name = ?", (acc_name,))
            conn.commit()
    ...
    def add_active_trade(self, trade: Dict):
        with sqlite3.connect(SQLITE_DB_PATH) as conn:
            ...
            """, (
                trade["id"], trade["symbol"], trade["market"], trade["account"],
                trade["strat"], trade["type"], trade["entry"], trade["sl"],
                trade["tp"], trade["qty"], trade["trail_sl"], trade["ts_trigger"],
                trade["opened_at"], trade["time"]
            ))
            conn.commit()
    ...
    def close_active_trade(self, trade_id: str, closed_data: Dict):
        with sqlite3.connect(SQLITE_DB_PATH) as conn:
            ...
            """, (
                closed_data["id"], closed_data["symbol"], closed_data.get("market", ""),
                closed_data["account"], closed_data["strat"], closed_data["type"],
                closed_data["entry"], closed_data["exit_price"], closed_data["pnl"],
                closed_data["result"], closed_data.get("exit_reason", ""),
                closed_data.get("close_time", ""), closed_data.get("closed_at", "")
            ))
            conn.commit()
    ...
    def update_trade_trail_sl(self, trade_id: str, new_trail_sl: float):
        with sqlite3.connect(SQLITE_DB_PATH) as conn:
            cur = conn.cursor()
            cur.execute("UPDATE active_trades SET trail_sl = ? WHERE id = ?", (new_trail_sl, trade_id))
            conn.commit()
    ...
    def get_pending_sweeps(self) -> List[Dict]:
        with sqlite3.connect(SQLITE_DB_PATH) as conn:
            conn.row_factory = sqlite3.Row
            cur = conn.cursor()
            cur.execute("SELECT * FROM pending_sweeps WHERE status != 'expired' AND status != 'invalidated'")
            rows = cur.fetchall()
            results = []
            for r in rows:
                d = dict(r)
                if d.get("fvg_zone_low") is not None and d.get("fvg_zone_high") is not None:
                    d["fvg_zone"] = [d["fvg_zone_low"], d["fvg_zone_high"]]
                else:
                    d["fvg_zone"] = None
                results.append(d)
            return results
    ...
    def add_pending_sweep(self, sweep_data: Dict):
        ...
        with sqlite3.connect(SQLITE_DB_PATH) as conn:
            cur = conn.cursor()
            fvg_l = sweep_data["fvg_zone"][0] if sweep_data.get("fvg_zone") else None
            fvg_h = sweep_data["fvg_zone"][1] if sweep_data.get("fvg_zone") else None
            cur.execute("""
                INSERT OR REPLACE INTO pending_sweeps VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            """, (
                sid, sweep_data["symbol"], sweep_data["mtype"], sweep_data["direction"],
                sweep_data["sweep_high"], sweep_data["sweep_low"], sweep_data["sweep_open_ts"],
                sweep_data["sweep_close_ts"], sweep_data["created_at"], fvg_l, fvg_h,
                sweep_data.get("fvg_tf"), sweep_data["status"], sweep_data["target_account"]
            ))
            conn.commit()
        ...
        with sqlite3.connect(SQLITE_DB_PATH) as conn:
            ...
            if fvg_zone:
                cur.execute("""
                    ...
                    SET status = ?, fvg_zone_low = ?, fvg_zone_high = ?, fvg_tf = ?
                    WHERE id = ?
                """, (status, fvg_zone[0], fvg_zone[1], fvg_tf, sid))
            else:
                cur.execute("UPDATE pending_sweeps SET status = ? WHERE id = ?", (status, sid))
            conn.commit()

    def remove_pending_sweep(self, sid: str):
        with sqlite3.connect(SQLITE_DB_PATH) as conn:
            cur = conn.cursor()
            cur.execute("DELETE FROM pending_sweeps WHERE id = ?", (sid,))
            conn.commit()
    ...
    def check_and_increment_signal(self, sig_key: str, max_count: int = 2) -> bool:
        ...
        with sqlite3.connect(SQLITE_DB_PATH) as conn:
            cur = conn.cursor()
            cur.execute("SELECT send_count FROM sent_signals WHERE sig_key = ?", (sig_key,))
            row = cur.fetchone()
            if row:
                count = row[0]
                if count >= max_count:
                ...
                cur.execute("INSERT INTO sent_signals VALUES (?, 1, ?)", (sig_key, now_ms))
            conn.commit()
        return True

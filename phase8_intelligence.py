...
def enrich_trade(trade: Dict[str, Any]) -> Dict[str, Any]:
    """Return explainable metadata for one existing trade without changing it."""
    item = dict(trade)
...
def strategy_statistics(history: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Aggregate closed trades by strategy using only recorded trade results."""
...
def build_intelligence(history: Iterable[Dict[str, Any]], signals: Iterable[Dict[str, Any]] = ()) -> Dict[str, Any]:
    """Build the Phase 8A read-only intelligence payload."""
    ...
    signal_rows = [dict(s) for s in signals if isinstance(s, dict)]

...
def signal_view(signal: Dict[str, Any]) -> Dict[str, Any]:
    """Build an explainable signal card from recorded metadata only."""
    item = dict(signal)
    return {
        "id": item.get("id"),
        "symbol": item.get("sym", item.get("symbol", "—")),
        "direction": item.get("dir", item.get("direction", "NEUTRAL")),
        "strategy": item.get("strategy", "Unknown"),
        "time": item.get("time", ""),
        "date": item.get("date", ""),
        "status": item.get("status", "SIGNAL SAVED"),
...
def trade_view(trade: Dict[str, Any]) -> Dict[str, Any]:
    """Build a trade card from Phase 8A enrichment without changing the record."""
    item = dict(trade)
    ...
    return {
        ...
        "exit": item.get("exit", item.get("close_price")),
        "stop": item.get("sl", item.get("trail_sl")),
        "pnl_inr": _money(intelligence.get("pnl_inr", item.get("pnl", item.get("pnl_inr", 0)))),
        "outcome": intelligence.get("outcome", "FLAT"),
        "r_multiple": intelligence.get("r_multiple"),
        "has_stop": bool(intelligence.get("has_stop", False)),
        "has_exit": bool(intelligence.get("has_exit", False)),
...
def strategy_cards(stats: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Return compact strategy performance cards; unavailable metrics stay null."""
    ...
    for row in stats:
        ...
        cards.append({
            ...
            "flat": int(row.get("flat", 0) or 0),
            "win_rate_pct": row.get("win_rate_pct"),
            "pnl_inr": row.get("pnl_inr"),
            "expectancy_inr": row.get("expectancy_inr"),
            "profit_factor": row.get("profit_factor"),
            "avg_win_inr": row.get("avg_win_inr"),
...
def build_dashboard_intelligence(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Create the additive 8B view model from a Phase 8A payload."""

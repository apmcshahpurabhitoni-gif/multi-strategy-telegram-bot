...
def msg_trade_closed(trade, live, pnl, bal, is_long, hit_tp):
    ...
    return (
        ...
        f"{BR}\n"
        f"`{trade['symbol']}` | {'LONG' if is_long else 'SHORT'}\n"
...
def safe_send(chat_id, text, **kwargs):
    try:
        ...
        try:
            ...
            bot.send_message(chat_id, text.replace("*","").replace("`","").replace("_",""), parse_mode=None, **fallback_kwargs)
...
def register_pending_sweep(symbol, mtype, sweep):
    ...
    if now_ts - last_fired < 4 * 3600 * 1000:
        print(f"[SWEEP COOLDOWN] {symbol} {direction} — last fired {(now_ts-last_fired)/3600000:.1f}h ago")
        return
    with _lock:
    ...
    dot = "🟢" if is_bullish else "🔴"
    dir_label = "LONG 📈" if is_bullish else "SHORT 📉"
    sl_extreme = sweep_low if is_bullish else sweep_high
    msg = (
        f"{dot} *SWEEP — WAITING FOR FVG*\n"
        f"{BR}\n"
...
def execute(symbol, mtype, account, strat, sig_type, price, a1, a2, a3=None, fvg_entry=None):
    ...
    with _lock:
        ...
        sent_signals[key] = {
            ...
            "symbol": symbol, "sig_type": sig_type, "strat": strat,
            "account": account, "status": "open", "pnl": 0,
            "hint": "", "time_str": datetime.now(IST).strftime("%H:%M"),
        }
...
def cmd_balance(m):
    ...
    text = (
        ...
        f"{BR}\n"
        f"{'🟢' if ny else '🔴'} *NY Session:* `{'ACTIVE' if ny else 'INACTIVE'}`\n"
...
def cmd_clear(m):
    global active_trades, history, sent_signals
    with _lock:
        active_trades = []
        history = []
        sent_signals = {}
...
def cmd_nifty(m):
    chat_id = m.chat.id
    safe_send(chat_id, "📈 *Fetching Nifty 50 stock prices...*")
    def run():
        lines = []
        for sym, name in NIFTY_STOCKS:
            p = get_price(sym)
            price_str = f"₹{p:,.2f}" if p else "N/A"
            lines.append(f"• `{name}` ({sym}): {price_str}")
        text = (
            "📈 *Nifty 50 — Top 15 Stocks*\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            + "\n".join(lines) +
            "\n━━━━━━━━━━━━━━━━━━━━━━\n"
            "⏱ 4H sweep scanning active\n"
            "🎯 Signals routed to *Nifty* account"
        )
        safe_send(chat_id, text, parse_mode="Markdown")
    threading.Thread(target=run, daemon=True).start()
...
def cb(c):
    try:
        ...
        elif data == "menu_nifty":
            bot.answer_callback_query(c.id, "Loading Nifty stocks...")
            lines = []
            for sym, name in NIFTY_STOCKS:
                p = get_price(sym)
                price_str = f"₹{p:,.2f}" if p else "N/A"

...
"
        f"🛑 *Trailing SL Level:* `${trade['trail_sl']:,.4f}`
"
...
def init_accounts():
    ...
    # --- FIX: include sweep_4h in daily reset ---
...
def get_price(symbol):
    ...
    try:
        ...
        # --- FIX: normalise columns so df["Close"] works with MultiIndex ---
    ...
    while True:
        ...
        for trade in trades_copy:
            try:
                ...
                # --- FIX: CSV write outside lock so monitor thread doesn't stall others ---
                try:
    ...
    def run_scan():
        try:
            ...
            for symbol, mtype in MONITORED:
                ...
                # --- FIX: explicit check so neutral only gets no-signal assets ---
...
def cmd_active(m):
    try:
        ...
        for t in trades:
            ...
            # --- FIX: show trade even when price fetch fails ---
            if live:
                if is_long:
...
def cmd_close(m):
    try:
        ...
        # --- FIX: do all mutation inside lock, capture bal, then release before I/O ---
        with _lock:
            ...
            # --- FIX: map manual close to WIN/LOSS so stats count it ---
...
def cmd_export(m):
    try:
        ...
        with open(TRADE_LOG_CSV, 'r') as f:
            # --- FIX: guard against negative count ---
...
def handle_cb(c):
    ...
    # --- FIX: wrap answer_callback_query in try-except ---
    try:
        bot.answer_callback_query(c.id)
    except Exception as e:
        print(f"[ERR] Callback answer: {e}")

...
# Nifty 50 Stock Universe — 15 most liquid names
NIFTY_STOCKS = [
...
_last_yf_call = 0
_yf_min_delay = 8.0  # seconds between yf.download calls
    ...
    while True:
        try:
            for symbol, mtype in MONITORED:
                ...
                # UT Bot Signals — ONLY for non-NSE assets (Forex, Crypto, Gold)
                if not is_nse:
                    ut = check_ut(symbol)
                    if ut:
                ...
                # Sweep — ONLY for NSE assets (1h for indices, 4h for stocks)
                if is_nse:
                    if not is_nifty_open():
                        continue

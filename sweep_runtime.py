...
def _save_signal_event(symbol, mtype, result):
    try:
        rows = _load_signal_history(); rows = rows if isinstance(rows, list) else []
        close_ts = int(result.candle_end.timestamp() * 1000); event_id = _key(symbol, close_ts)
        if any(isinstance(x, dict) and x.get("id") == event_id for x in rows): return
        rows.append({"id": event_id, "symbol": symbol, "market": mtype, "direction": result.direction, "strategy": f"{result.timeframe} Sweep", "timeframe": result.timeframe, "candle_start": result.candle_start.isoformat(), "candle_end": result.candle_end.isoformat(), "created_at": datetime.now().astimezone().isoformat(), "reminder_sent": False})
...
def _freshness(main, candle_end):
    age_seconds = max(0.0, (datetime.now(main.IST) - candle_end).total_seconds()); age_minutes = int(age_seconds // 60)
...
def _price_decimals(symbol):
    ...
    if symbol in {"GC=F", "SI=F", "HG=F"}: return 2
    if symbol.endswith(".NS") or symbol in {"^NSEI", "^NSEBANK"}: return 2
    return 2


def _fmt_price(symbol, value, currency): return f"{currency}{float(value):,.{_price_decimals(symbol)}f}"


def _canonical_main_age(ts_ms):
    if not ts_ms: return "Unknown", "⚠️ STALE"
    diff_ms = max(0, int(time.time() * 1000) - int(ts_ms)); diff_min = int(diff_ms // 60000)
    if diff_ms <= 3600 * 1000: return f"{diff_min} min ago", "✅ FRESH"
...
def _signal_message(main, symbol, mtype, result, reminder=False, entry=None, sl=None, tp=None, account=None, qty=None, risk_amt=None):
    ...
    if account is not None and result.direction in {"BULLISH", "BEARISH"}: lines.append(f"🏢 *Account:* `{str(account).upper()}`")
    if qty is not None and result.direction in {"BULLISH", "BEARISH"}: lines.append(f"📦 *Quantity:* `{qty:.2f}`")
    if risk_amt is not None and result.direction in {"BULLISH", "BEARISH"}: lines.append(f"💸 *Risk:* `₹{risk_amt:,.2f}`")
    if result.schedule_warning: lines.append(f"⚠️ *Candle timing:* `{result.schedule_warning}`")
    if status == "STALE": lines.append("⚠️ *STALE — older than 1 hour; no new trade should be opened.*")
    lines.extend([main.BR, _source_warning(symbol), main.BR2]); return "\n".join(lines)


def install(main):
    global CONTEXT, STARTUP_BASELINE, _MAIN
    _MAIN = main; _load(); CONTEXT = {}; STARTUP_BASELINE = {}
    try: _load_signal_history()
    ...
    original_msg = main.msg_trade_signal; main.get_signal_age_str = _canonical_main_age
    ...
    def check_sweep_v2(symbol, df=None):
        try:
            ...
            latest_close = latest_start + (timedelta(hours=1) if symbol in ("^NSEI", "^NSEBANK") else timedelta(hours=2) if symbol.endswith(".NS") and getattr(latest_start, "hour", None) == 13 else timedelta(hours=4))
            if symbol not in STARTUP_BASELINE: STARTUP_BASELINE[symbol] = int(latest_close.timestamp() * 1000); return None
            result = detect_sweep(df, symbol, now)
            if result is None: return None
            close_ts = int(result.candle_end.timestamp() * 1000); open_ts = int(result.candle_start.timestamp() * 1000)
            if int(now.timestamp() * 1000) - close_ts > 3600 * 1000 or close_ts <= STARTUP_BASELINE[symbol]: return None
            CONTEXT[_key(symbol, close_ts)] = result; return (result.direction, result.current["High"], result.current["Low"], close_ts, open_ts, result)
    ...
    def msg_trade_signal_v2(symbol, mtype, strat, sig_type, tf, price, sl, tp, qty, risk_amt, account, signal_ts_ms):
        if strat and "Sweep" in strat:
            ...
            if result is not None: return _signal_message(main, symbol, mtype, result, entry=price, sl=sl, tp=tp, account=account, qty=qty, risk_amt=risk_amt)
    ...
    def _send_reminder(symbol, mtype, result):
        ...
        main.send_sweep_to_all(_signal_message(main, symbol, mtype, result, reminder=True), parse_mode="Markdown"); state["reminder"] = True; state["reminder_sent"] = int(time.time() * 1000); _save()
        try:
    ...
    def handle_sweep_v2(symbol, mtype, sweep):
        ...
        state.update({"initial": True, "direction": direction, "timeframe": result.timeframe, "candle_start": result.candle_start.isoformat(), "candle_end": result.candle_end.isoformat(), "high": result.current["High"], "low": result.current["Low"]}); _save(); CONTEXT[key] = result; _save_signal_event(symbol, mtype, result)
    ...
    main.check_sweep = check_sweep_v2; main.handle_sweep = handle_sweep_v2; main.notify_neutral_sweep = notify_neutral_v2; main.msg_trade_signal = msg_trade_signal_v2; main.SWEEP_ENGINE_VERSION = "v2.3"; main.SWEEP_RULE = "closed candle: current high > previous high AND current low < previous low; close classifies BUY/NEUTRAL/SELL"; main.SWEEP_DATA_WARNING = True
...
def _load_signal_history():
    ...
    if fn:
        try:
            rows = fn(SIGNAL_HISTORY_FILE, []); return rows if isinstance(rows, list) else []
        except Exception: pass
    try:
        with open(SIGNAL_HISTORY_FILE, "r", encoding="utf-8") as f: rows = json.load(f); return rows if isinstance(rows, list) else []
...
def _write_signal_history(rows):
    ...
    if fn:
        try: fn(SIGNAL_HISTORY_FILE, rows); return
        except Exception: pass
    try:
        os.makedirs(os.path.dirname(SIGNAL_HISTORY_FILE), exist_ok=True); tmp = SIGNAL_HISTORY_FILE + ".tmp"

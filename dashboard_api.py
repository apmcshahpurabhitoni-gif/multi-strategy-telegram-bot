...
def _load_file(main, path, default):
    try:
        ...
        print("[DASHBOARD] load:", e)
    try:
        if os.path.exists(path):
            with open(path, "r", encoding="utf-8") as f:
                return json.load(f)
    except Exception as e:
        print("[DASHBOARD] file load:", e)
    return default
...
def _parse_ts(value, default=0):
    ...
    s = str(value).strip()
    try:
        return int(float(s)) if float(s) > 10_000_000_000 else int(float(s) * 1000)
    except Exception:
        pass
    try:
    ...
    if lock:
        with lock:
            for s, v in cache.items():
                if s in symbols and isinstance(v, (list, tuple)) and len(v) >= 2 and now - v[1] < 120:
                    out[s] = v[0]
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                if len(need) == 1:
                    vals = close.dropna()
                    if not vals.empty:
                        out[need[0]] = float(vals.iloc[-1])
                else:
                    for s in need:
                        try:
                            vals = close[s].dropna()
                            if not vals.empty:
                                out[s] = float(vals.iloc[-1])
                        except Exception:
                            pass
        except Exception as e:
            print("[DASHBOARD] price fetch:", e)
    if get_price:
        for s in symbols:
            if s not in out:
                try:
...
def _build_equity_curve(history, starting=400000.0, days=60):
    ...
    for t in history:
        ...
        if not d:
            continue
        try:
            daily[d] = daily.get(d, 0) + float(t.get("pnl", 0) or 0)
        except Exception:
            pass
    ...
    for d in sorted(daily):
        ...
        if dd > ddmax:
            ddmax = dd
            ddpct = dd / peak * 100 if peak else 0
...
def _build_actual_signals(main, ist, cutoff):
    """Read the canonical sweep_runtime state, not trade history or sent-message counters."""
    state_path = getattr(main, "SWEEP_RUNTIME_STATE_FILE", "/tmp/workspace/sweep_runtime_state.json")
    try:
        import sweep_runtime
        state_path = getattr(sweep_runtime, "STATE_FILE", state_path)
        runtime_state = getattr(sweep_runtime, "STATE", None) or {}
    except Exception:
        runtime_state = {}
    persisted = _load_file(main, state_path, {})
    if isinstance(persisted, dict):
        if len(persisted) > len(runtime_state):
    ...
    for key, record in runtime_state.items():
        if not isinstance(record, dict) or not record.get("initial"):
            continue
        ...
        signals.append({
            ...
            "dir": direction,
            "strategy": strategy,
            "status": "REMINDER SENT" if record.get("reminder") else "FRESH",
            "pnl": 0,
            "hint": "Actual sweep signal",
            "ts_ms": ts,
            "reminder": bool(record.get("reminder")),
        })
...
def _build_snapshot():
    ...
    for t in history:
        if str(t.get("closed_at", t.get("close_time", "")))[:10] == today:
            try:
                acc = t.get("account")
                per_today[acc] = per_today.get(acc, 0) + float(t.get("pnl", 0) or 0)
            except Exception:
                pass
    ...
    for k, a in accounts.items():
        if isinstance(a, dict):
            accounts_view[k] = {
                ...
                "daily_trades": int(a.get("daily_trades", 0) or 0),
                "daily_limit": int(limits.get(k, 0) or 0),
                "today_pnl": round(per_today.get(k, 0), 2),
            }

    symbols = [t.get("symbol") for t in active if t.get("symbol")]
    ...
    for t in active:
        ...
        sl = float(t.get("sl", t.get("trail_sl", 0)) or 0)
        tp = float(t.get("tp", 0) or 0)
        typ = str(t.get("type", t.get("direction", "LONG"))).upper()
        long = "LONG" in typ or "BULL" in typ or typ == "BUY"
        cur = float(prices.get(sym, entry) or entry)
    ...
    # Your rule: a signal is valid on the dashboard for one hour after the closed candle.
    ...
    try:
        ...
        print("[DASHBOARD] cached news:", e)
    if not news and fetch:
        try:
            news = fetch() or []
        except Exception as e:
            print("[DASHBOARD] fetch news:", e)
    normalized_news = []
    for ev in news[:120]:
        if isinstance(ev, dict):
            x = dict(ev)
            x["impact"] = str(x.get("impact") or x.get("importance") or "LOW").upper()
            normalized_news.append(x)
    ...
    return {
        ...
        "equity_curve": curve,
        "risk": {"max_drawdown_inr": curve["max_drawdown_inr"], "max_drawdown_pct": curve["max_drawdown_pct"]},
...
def _get_snapshot_cached():
    ...
    with _snapshot_lock:
        _snapshot_cache["data"], _snapshot_cache["ts"] = snap, now
...
def _route_backtest(start_response, environ):
    try:
        ...
        days = max(7, min(int(q.get("days", ["30"])[0] or 30), 730))
        if not symbol:
            return _json_response(start_response, {"error": "symbol required"}, "400 Bad Request")
        from backtest import BacktestEngine
        eng = BacktestEngine()
        res = eng.backtest_sweep(symbol, days) if strategy == "sweep" else eng.backtest_trendpulse(symbol, days)
        if not isinstance(res, dict):
            res = {"error": "backtest failed"}
        if "error" not in res:
            res = {"metrics": res, "symbol": symbol, "strategy": strategy, "days": days}
        else:
            res.update({"symbol": symbol, "strategy": strategy, "days": days})
...
def _route_close_trade(start_response, environ):
    try:
        ...
        data = json.loads(environ["wsgi.input"].read(n).decode()) if n else {}
        ...
        ok, msg = fn(tid, reason="Dashboard") if fn else (False, "Not found")
        return _json_response(start_response, {"success": ok, "message": msg})
    except Exception as e:
        return _json_response(start_response, {"success": False, "error": str(e)}, "500 Internal Server Error")
...
def _route_refresh_news(start_response, environ):
    ...
    try:
        ...
        return _json_response(start_response, {"ok": False, "error": str(e)}, "500 Internal Server Error")

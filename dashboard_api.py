...
def _build_actual_signals(main, ist):
    """Permanent dashboard signal history. One-hour duplicate protection is Telegram-only."""
...
def _build_snapshot():
    ...
    accounts = getattr(main, "accounts", {}) or {}; active = getattr(main, "active_trades", []) or []; limits = getattr(main, "ACCOUNT_LIMITS", {}) or {}
    per_today = {k: 0.0 for k in accounts}
    for t in history:
        if str(t.get("closed_at", t.get("close_time", "")))[:10] == today:
            try:
                acc=t.get("account"); per_today[acc]=per_today.get(acc,0)+float(t.get("pnl",0) or 0)
            except Exception: pass
    accounts_view={}
